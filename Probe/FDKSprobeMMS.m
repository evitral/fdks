...
for y = 0:dY:(L-dY)
    ...
    for x = 0:dX:(L-dX)
        i=i+1;
        hn(j,i) = ho+hx*sin(ax*pi()*x/(L))+hy*cos(ay*pi()*y/(L)) ...
        +hxy*sin(ax*pi()*x/(L))*cos(ay*pi()*y/(L))*exp(b*double(dtau*0));
    end
end
...
L2p1  = zeros(1,ntau/tdata);
L2p2  = zeros(1,ntau/tdata);
L2p3  = zeros(1,ntau/tdata);
L2p4  = zeros(1,ntau/tdata);
L2p5  = zeros(1,ntau/tdata);
L2p6  = zeros(1,ntau/tdata);
L2p7  = zeros(1,ntau/tdata);
L2p8  = zeros(1,ntau/tdata);
L2p9  = zeros(1,ntau/tdata);
L2p10 = zeros(1,ntau/tdata);
L2p11 = zeros(1,ntau/tdata);
L2p12 = zeros(1,ntau/tdata);
L2p13 = zeros(1,ntau/tdata);
L2p14 = zeros(1,ntau/tdata);
L2p15 = zeros(1,ntau/tdata);
L2p16 = zeros(1,ntau/tdata);
...
%---------------------------------------------------------------%
    ...
    siy = sin(ay*pi()*y/(L));
        ...
        MMS(j,i) = ebt*b*hxy*coy*six+alpha*(ho+hy*coy+hx*six+ebt*hxy*coy*six) ...
...
%---------------------------------------------------------------%
       ...
       MMX = dtau*MMS(:,:);
        ...
        for lin = 1:np
             htil(lin,:) = M1i*(M3*hn(lin,:)'+2*Lyh(lin,:)' + Mf(lin,:)' + MMX(lin,:)');
        end
...
%---------------------------------------------------------------% 
    ...
    for x = 0:dX:(L-dX)
        i=i+1;
        hw(j,i) = ho+hx*sin(ax*pi()*x/(L))+hy*cos(ay*pi()*y/(L)) ...
        +hxy*sin(ax*pi()*x/(L))*cos(ay*pi()*y/(L))*exp(b*double(t)*dtau);
    end
    end
...
%     L2p1(t)  = ((hm1(1,1)-hw(1,1)).^2/Nm).^0.5;
%     L2p2(t)  = ((hm1(1,pb1)-hw(1,pb1)).^2/Nm).^0.5;
%     L2p3(t)  = ((hm1(1,pb2)-hw(1,pb2)).^2/Nm).^0.5;
%     L2p4(t)  = ((hm1(1,np)-hw(1,np)).^2/Nm).^0.5;
%     L2p5(t)  = ((hm1(pb1,1)-hw(pb1,1)).^2/Nm).^0.5;
%     L2p6(t)  = ((hm1(pb1,pb1)-hw(pb1,pb1)).^2/Nm).^0.5;
%     L2p7(t)  = ((hm1(pb1,pb2)-hw(pb1,pb2)).^2/Nm).^0.5;
%     L2p8(t)  = ((hm1(pb1,np)-hw(pb1,np)).^2/Nm).^0.5;    
%     L2p9(t)  = ((hm1(pb2,1)-hw(pb2,1)).^2/Nm).^0.5;
%     L2p10(t) = ((hm1(pb2,pb1)-hw(pb2,pb1)).^2/Nm).^0.5;
%     L2p11(t) = ((hm1(pb2,pb2)-hw(pb2,pb2)).^2/Nm).^0.5;
%     L2p12(t) = ((hm1(pb2,np)-hw(pb2,np)).^2/Nm).^0.5;    
%     L2p13(t) = ((hm1(np,1)-hw(np,1)).^2/Nm).^0.5;
%     L2p14(t) = ((hm1(np,pb1)-hw(np,pb1)).^2/Nm).^0.5;
%     L2p15(t) = ((hm1(np,pb2)-hw(np,pb2)).^2/Nm).^0.5;
%     L2p16(t) = ((hm1(np,np)-hw(np,np)).^2/Nm).^0.5;       
