...
alpha = 0.6;      % unclear physical meaning (B&H, alpha = 0)
...
%dtau = 2*F*amu^2*dt/a;
dtau = 1.0;   %OBS: posso estar usando um valor diferente do usual aqui
ntau = 300;
tmax = ntau*dtau;
...
eta = nuxt/(4*dX^2);   % (dh/dX)^2
psi = c^3/(4*dY^2);   % (dh/dY)^2
...
%---------------------------------------------------------------%
    ...
    cot = cos(at*pi()*(double(t)*dtau)/L);
    sit = sin(at*pi()*(double(t)*dtau)/L);
...
%---------------------------------------------------------------%
    ...
    for x = 0:dX:L
        ...
        +hxy*sin(ax*pi()*x/L)*cos(ay*pi()*y/L)*exp(b*double(dtau*t))...
        +ht*sin(at*pi()*(double(t)*dtau)/L);
    end
    end
