...
%---------------------------------------------------------------%
%    Finite Differences Numerical Scheme for Solving a          %
%    Generalized Kuramoto-Sivashinsky Equation Modelling        %
%    Nano-Patterning of Surfaces by Ion Sputtering              %
%                                                               %
%    Method of Manufactured SolutionsMS:                        %
...
F = (J*ei*p/sqrt(2*pi()*mu))*exp(-amu^2*c^2/2);
...
%---------------------------------------------------------------%
       ...
       htil(1,:) = M1i*(M3*hn(1,:)'+2*Lyh1' + Mf(1,:)'+MMX(1,:)');
       htil(2,:) = M1i*(M3*hn(2,:)'+2*Lyh2' + Mf(2,:)'+MMX(2,:)');
       htil(npm,:) = M1i*(M3*hn(npm,:)'+2*Lyhnpm' + Mf(npm,:)'+MMX(npm,:)');
       htil(np,:) = M1i*(M3*hn(np,:)'+2*Lyhnp' + Mf(np,:)'+MMX(np,:)');
        ...
        for lin = 3:npmm
            ...
            htil(lin,:) =M1i*(M3*hn(lin,:)'+2*Lyh'+Mf(lin,:)'+MMX(lin,:)');
        end
...
%---------------------------------------------------------------% 
    ...
    for x = 0:dX:L
        ...
        +hxy*sin(ax*pi()*x/(L+dX))*cos(ay*pi()*y/(L+dX))*exp(b*double(t)*dtau);
    end
    end
