...
alpha = .15;       % unclear physical meaning (B&H, alpha = 0)
...
eta = nuxt/(2*dX^2);   % (dh/dX)^2   MUDDEEEEEEEEEI  ERA 4
psi = c^3/(2*dY^2);   % (dh/dY)^2    MUDEEEEIIIIII     ERA 4
...
% end
    ...
    for x = 0:dX:L
        i=i+1;
        hn(j,i) = ho+hxy*sin((ax*pi()*x+ay*pi()*y+at*0)/(L+dX))^2;
    end
    end
...
%---------------------------------------------------------------%
    ...
    j = j+1;
        ...
        cot = cos((ax*pi()*x+ay*pi()*y+at*double(t)*dtau)/(L+dX))^2;
        sit = sin((ax*pi()*x+ay*pi()*y+at*double(t)*dtau)/(L+dX))^2;
        
        MMS(j,i) = 2*at*hxy*cot*sit/(L+dX)...
        +alpha*(ho+hxy*sit^2) ...
        -mut*2*hxy*(pi()*ax/(L+dX))^2*(cot^2-sit^2)...
        +c^2*2*hxy*(pi()*ay/(L+dX))^2*(cot^2-sit^2)...
        -nuxt*((2*ay*hxy*pi()*cot*sit)/(L+dX))^2 ...
        +c^3*((2*ay*hxy*pi()*cot*sit)/(L+dX))^2 ...
        +Dxx*8*hxy*(ax*pi()/(L+dX))^4*(sit^2-cot^2)...
        -Dxy*8*hxy*(ax*ay)^2*(pi()/(L+dX))^4*(sit^2-cot^2)+...
        -c^2*8*hxy*(ay*pi()/(L+dX))^4*(sit^2-cot^2)...
        +Kt*(8*hxy*(ax*pi()/(L+dX))^4*(sit^2-cot^2)...
        +16*hxy*(ax*ay)^2*(pi()/(L+dX))^4*(sit^2-cot^2)...
        +8*hxy*(ay*pi()/(L+dX))^4*(sit^2-cot^2));
...
%---------------------------------------------------------------%
    ...
    for x = 0:dX:L
        i=i+1;
        hw(j,i) = ho+hxy*sin((ax*pi()*x+ay*pi()*y+at*double(t)*dtau)/(L+dX))^2;
    end
    end
